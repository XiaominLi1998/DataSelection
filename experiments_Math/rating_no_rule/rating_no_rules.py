...
if __name__ == "__main__":
    ...
    texts = []
    print("Loading data...")
    with open("DataSelections/SlimPajama_data/SlimPajama6B_short_1M.jsonl", 'r') as jsonl_file:
    ...
    print("\n\n==============Fetch no_rule rating prompt:==================\n")

    def no_rule_rating(batch_data):
        size = len(batch_data)
        questions = []
        for i in range(size):
            prompt_i = rating_prompt(batch_data[i])
    ...
    for start_idx in range(0, total_size, batch_size):
        ...
        if os.path.exists(batch_save_path):
            print(f"Batch {batch_number} for no_rule already exists, skipping.")
            continue
    
        print(f"\nProcessing batch {batch_number} for no_rule with size {len(batch_data)}"); sys.stdout.flush()
        
        # Compute ratings for the current batch
        quality_responses = no_rule_rating(batch_data)
